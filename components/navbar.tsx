...
import { IconSearch, IconCommand, IconMenu2 } from "@tabler/icons-react";
...
const Navbar = ({ onMenuToggle }: NavbarProps) => {
    ...
    return (
        <nav className="sticky top-0 z-50 flex items-center justify-between px-4 md:px-6 py-3 border-b bg-background/95 backdrop-blur supports-[backdrop-filter]:bg-background/60">
            <div className="flex items-center gap-3">
                <Button
                    variant="ghost"
                    size="icon"
                    className="md:hidden rounded-sm text-muted-foreground"
            ...
            <div className="flex items-center gap-4">
                <button className="hidden md:flex relative group items-center gap-2 h-8 w-64 rounded-sm border border-input bg-muted/40 px-3 text-sm text-muted-foreground hover:bg-accent transition-colors">
